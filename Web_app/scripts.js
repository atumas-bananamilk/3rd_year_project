/*
  ...
  3rd Year Project
*/
/* Javascript functions that are mostly related to processing 3D shapes (mainly on app.php). */
...
// holds all the info about the current shape (which consists of layers)
var layers = [];
...
function load_JS(layers_given){
	...
	/* DEPRECATED: not being used in the newest version of the project. */
	// clear_result();

	// the main 3D shape rendering function
	function render(){
...
/*
	...
	Current idea (all the code above): create a shape of 9 layers with 
		...
		would then be encoded and sent to Arduino by statically turning 
		on and leaving the LEDs so you see a cylindrical shape without 
		interacting with the LEDs (turning them on/off periodically).

	Initial idea (code above + code below): generate invisible shapes 
		that would be used for collision detection after drawing a new 
		shape - line. Those collisions are then transformed (encoded) 
		into an array of coordinates which are then being sent to 
		Arduino. Everything worked, but the equirements changed because 
		of the issues (hardware) with the LED refresh rates.
	---------------------------------------------------------------------
*/
