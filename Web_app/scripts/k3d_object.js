...
(function()
{
   ...
   K3D.BaseObject.prototype =
   {
      ...
      rminx: 0, rminy: 0, rmaxx: 0, rmaxy: 0,
      doublesided: false,
...
(function()
{
   ...
   extend(K3D.K3DObject, K3D.BaseObject,
   {
      ...
      //fillstroke: true,       // DECRECATED: true to fill and then stroke solid polygons - else just fill
      perslevel: 1024,        // perspective level multiplier - powers of 2 recommended
      ...
      {
         var x, y, z,
             worldcoords = this.worldcoords, screencoords = this.screencoords,
             screenx = this.screenx, screeny = this.screeny, perslevel = this.perslevel;

         // reset render boundries
         this.rminx = this.rminy = 4096;
         this.rmaxx = this.rmaxy = -4096;
         
         // perform simple perspective transformation
         for (var i=0,sx,sy,len=this.points.length; i<len; i++)
         {
            x = worldcoords[i].x;
            y = worldcoords[i].y;
            ...
            // calculate final screencoords with perspective calculation
            sx = screencoords[i].x = ((x * perslevel) / z) + screenx;
            sy = screencoords[i].y = screeny - ((y * perslevel) / z);
            
            // update min/max rendering boundries
            if (sx < this.rminx) this.rminx = sx;
            if (sx > this.rmaxx) this.rmaxx = sx;
            if (sy < this.rminy) this.rminy = sy;
            if (sy > this.rmaxy) this.rmaxy = sy;
         }
      },
